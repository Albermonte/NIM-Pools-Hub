...
export const actions = {
  async UPDATE_USER_INFO({ commit, rootState }) {
    const info = await this.$axios.$get(
      ...
      }/api/siriuspool/${rootState.localStorage.address
        .replace(/(.{4})/g, "$1 ")
        .slice(0, -1)}`
    );
    if (info === "offline") {
      console.error(
        "Siriuspool USER_INFO not responding ",
  ...
  async UPDATE_POOL_INFO({ commit }) {
    const info = await this.$axios.$get(
      `${window.location.origin}/api/stats/siriuspool`
    );
    if (info === "offline") {
      console.error(
        "Siriuspool POOL_INFO not responding ",
        new Date().toUTCString()
      );
      commit(
        "poolInfoError",
