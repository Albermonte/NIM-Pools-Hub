...
export const actions = {
  async UPDATE_USER_INFO({ commit, rootState }) {
    const info = await this.$axios.$get(
      `${window.location.origin}/api/hashexpress/${rootState.localStorage.address}`
    );
    if (info === "offline") {
      console.error(
        "HashExpress USER_INFO not responding ",
  ...
  async UPDATE_POOL_INFO({ commit }) {
    const info = await this.$axios.$get(
      `${window.location.origin}/api/stats/hashexpress`
    );
    if (info === "offline") {
      console.error(
        "HashExpress POOL_INFO not responding ",
        new Date().toUTCString()
      );
      commit(
        "poolInfoError",
