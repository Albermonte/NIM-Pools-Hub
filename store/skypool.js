...
export const actions = {
    async UPDATE_USER_INFO({ commit, rootState }) {
        const info = await this.$axios.$get(`${window.location.origin}/api/skypool/${rootState.localStorage.address.replace(/(.{4})/g, "$1 ").slice(0, -1)}`)
        if (info === 'offline') {
    ...
    async UPDATE_POOL_INFO({ commit }) {
        const info = await this.$axios.$get(`${window.location.origin}/api/stats/skypool`)
        if (info === 'offline') {
            console.error('Skypool POOL_INFO not responding ', (new Date).toUTCString())
