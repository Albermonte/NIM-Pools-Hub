...
export const actions = {
  async UPDATE_USER_INFO({ commit, rootState }) {
    const info = await this.$axios.$get(
      `${window.location.origin}/api/e4pool/${rootState.localStorage.address}`
    );
    if (info === "offline") {
      console.error(
        "e4pool Pool USER_INFO not responding ",
  ...
  async UPDATE_POOL_INFO({ commit }) {
    const info = await this.$axios.$get(
      `${window.location.origin}/api/stats/e4pool`
    );
    if (info === "offline") {
      console.error(
        "e4pool Pool POOL_INFO not responding ",
        new Date().toUTCString()
      );
      commit(
        "poolInfoError",
