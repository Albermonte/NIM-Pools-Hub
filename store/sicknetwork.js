...
export const actions = {
  async UPDATE_USER_INFO({ commit, rootState }) {
    const info = await this.$axios.$get(
      `${window.location.origin}/api/sicknetwork/${rootState.localStorage.address}`
    );
    if (info === "offline") {
      console.error(
        "NIMIQ SICK NETWORK USER_INFO not responding ",
  ...
  async UPDATE_POOL_INFO({ commit }) {
    const info = await this.$axios.$get(
      `${window.location.origin}/api/stats/sicknetwork`
    );
    if (info === "offline") {
      console.error(
        "NIMIQ SICK NETWORK POOL_INFO not responding ",
        new Date().toUTCString()
      );
      commit(
        "poolInfoError",
