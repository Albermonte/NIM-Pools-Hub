...
export const actions = {
  async UPDATE_USER_INFO({ commit, rootState }) {
    const info = await this.$axios.$get(
      `${window.location.origin}/api/acemining/${rootState.localStorage.address}`
    );
    if (info === "offline") {
      console.error(
        "Acemining USER_INFO not responding ",
  ...
  async UPDATE_POOL_INFO({ commit }) {
    const info = await this.$axios.$get(
      `${window.location.origin}/api/stats/acemining`
    );
    if (info === "offline") {
      console.error(
        "Acemining POOL_INFO not responding ",
        new Date().toUTCString()
      );
      commit(
        "poolInfoError",
